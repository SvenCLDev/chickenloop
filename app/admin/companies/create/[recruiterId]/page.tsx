...
export default function AdminCreateCompanyPage() {
  ...
  const [ownerRecruiter, setOwnerRecruiter] = useState<{ name: string; email: string } | null>(null);
  ...
  useEffect(() => {
    ...
    } else if (user && user.role === 'admin' && recruiterId) {
      loadOwnerRecruiter();
    } else if (user && user.role === 'admin' && !recruiterId) {
      router.replace('/admin/companies/new');
    }
  }, [user, authLoading, router, recruiterId]);

  const loadOwnerRecruiter = async () => {
    setFetching(true);
    try {
      ...
      if (recruiter.companyId) {
        throw new Error('This recruiter already has a company');
      }
      setOwnerRecruiter({ name: recruiter.name, email: recruiter.email });
      setFormData((prev) => ({
        ...prev,
        contact: {
          ...prev.contact,
          email: recruiter.email || prev.contact.email,
        },
      }));
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to load recruiter');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminApi.createCompany(recruiterId, payload);
      ...
      setTimeout(() => {
        router.push('/admin?section=companies');
      }, 3000);
    } catch (err: unknown) {
      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold mb-2 text-gray-900">Create Company</h1>
            <p className="text-gray-600">
              {ownerRecruiter ? (
                <>
                  Owner: <span className="font-medium text-gray-900">{ownerRecruiter.name}</span> (
                  {ownerRecruiter.email})
                </>
              ) : (
                'Complete the company profile for the selected recruiter.'
              )}
            </p>
          </div>
          {error && (
