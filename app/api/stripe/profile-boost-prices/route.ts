...
const CV_BOOST_LOOKUP_KEYS = ['featured_cv_7', 'featured_cv_14', 'featured_cv_30'] as const;

export interface CvBoostPriceItem {
  lookupKey: string;
  durationDays: number;
  priceId: string;
  amount: number;
...
/** Parse duration days from lookup_key (e.g. featured_cv_7 -> 7). */
...
export async function GET() {
  ...
  try {
    const response = await stripe.prices.list({
      active: true,
      lookup_keys: [...CV_BOOST_LOOKUP_KEYS],
      expand: ['data.product'],
    });
    ...
    const missing = CV_BOOST_LOOKUP_KEYS.filter((key) => !byLookupKey.has(key));
    if (missing.length > 0) {
      return NextResponse.json(
        {
    ...
    const items: CvBoostPriceItem[] = CV_BOOST_LOOKUP_KEYS.map((lookupKey) => {
